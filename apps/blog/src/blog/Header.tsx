...
import { GlobalIcon, GitHubIcon } from "../../../pricing/src/components/HeaderIcons";
import { AnimatedThemeToggler } from "../../../pricing/src/components/AnimatedThemeToggler";
import "./header.css";
...
const links = [
  ...
  ["Pricing", "定价", import.meta.env.DEV ? "http://localhost:3003/#overview" : "https://knowhere-pricing.knowhere-landing.workers.dev/#overview"],
...
export function Header() {
  ...
  return (
    <header ref={header} className={`blog-site-header${scrolled ? " scrolled" : ""}${menuOpen ? " menu-open" : ""}`}
      onKeyDown={(event) => {
        if (event.key === "Escape") {
      ...
      <nav className="kb-shell header-nav" aria-label="Main navigation">
        ...
        <div className="header-links">
          {links.map(([en, cn, href]) => <a key={en} href={href} target={en === "Docs" || en === "Blog" ? "_blank" : undefined} rel={en === "Docs" || en === "Blog" ? "noopener noreferrer" : undefined}>{zh ? cn : en}</a>)}
        </div>
