...
`--serif`、`--figma-display`、`--figma-mono` 是兼容变量名，分别引用上述字体栈。样式控制器、中英文切换、Canvas 文字及独立 HTML 入口均遵守同一字体分工；`--mono` 独立于 Sans 设置。图标字体只用于图标。
...
| 模块 | 规则 |
| --- | --- |
| 中英文 | 英文与数字按文字角色使用 Geist 或 Geist Mono；中文使用 Frex；章节英文标签、品牌名与代码保留其表达形式 |
| 导航菜单 | 提供打开和关闭状态；支持 Escape 关闭及焦点返回 |
| SDK 示例 | Python、Node.js、CURL 切换对应代码；保留等宽对齐与代码区滚动；复制反馈对应所选代码 |
