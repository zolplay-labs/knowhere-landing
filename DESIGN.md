...
当前局部 Mono 作用域包括章节标签、导航主按钮、Product 终端代码、SDK 代码框（含 Tab 与复制反馈）。这些局部设置不扩展到其他文字。导航主按钮目前的字体栈为 `"Geist Mono", monospace`，缺少 Frex 中文回退；其余 Mono 栈见下表。
...
三个格式的 API 输出框顶部仅保留窗口圆点。手机、平板和触屏设备上，代码按内容自然展开，随页面纵向滚动；仅在宽度 ≥1200px 且使用精细指针、支持悬停时，Word / Excel 输出框限制为 440px 并允许内部滚动。
...
| Hero 次按钮悬停 / 按下 | black 3% / 6% | 当前为 surface 6% / 10%，见 §2.3 |
...
暗色 `--black-N` 指 on-surface 透明色，不等于 `--white-N`。Hero 次按钮当前引用 `--white-6 / 10`，因此实际为暗色 surface；旧文档中的 on-surface 描述与实现不符。
...
| 顶栏主按钮悬停 | `#7D817C` | primary 92% 与 on-primary 混合 |
| 导航主按钮按下 | 未单独定义 | primary 88% 与 on-primary 混合 |
| 页脚链接悬停 | `#12846C` | `#1DBE9D` |
| 页脚大字标底色 | ink 3% | ink 3% |

桌面顶栏初始底线固定为黑色 6%；滚动后及 ≤1199 时使用 line。语言菜单选中底为黑色 6%，悬停／焦点底为黑色 3%，当前在两种主题下保持该值。亮色菜单内主按钮未单独定义悬停态；暗色与顶栏主按钮一致。
...
| Hero 按钮 | 44 | 1 / 1 | 15 / 24 · 500 | 16 | 手机两按钮同行等宽，gap 12 |
...
Landing 已实现通用按钮颜色、按下与禁用样式；加载图标属于表单组件规范。导航 `.kh-button` 使用 §4.3，不继承 `.button` 状态。
...
输入框状态优先级：禁用 → 聚焦 → 错误 → 悬停 → 默认。标签始终可见，错误反馈位于对应输入框下方。上述输入框禁用底色已在参考 HTML 中体现；Login 当前仍为 mist-white/300，白按钮按下色也仍为 mist-white/300。
...
| 悬停 | 透明底，mineral-green/700 文字与描边 | 设计目标；当前被基础样式覆盖 |
| 选中 | mineral-green/500 实底及描边，白字 | 设计目标；当前被基础样式覆盖 |
| 按下 | 控件按下底色，保留悬停文字与描边 | 参考 HTML 已体现 |
| 键盘焦点 | 主色 2 外轮廓，间隔建议 2 | 扩展建议 |
| 禁用 | 底 black/10、字 black/40、边 black/3 | 扩展建议 |

颜色过渡 140ms。暗色使用 §2 的主题 token；当前选中态声明引用 `--white-100`，该变量在暗色中为 surface，不等于固定白字。交互使用普通按钮和 `aria-pressed`；完整 Tab 键盘模型见 §5 的扩展建议。
...
| Logo 宽 | 112 | 120 | 108 |
| 导航链接 | 横排；高 36，水平 padding 12，圆角 10，gap 8 | 收入菜单 | 收入菜单 |
| 顶栏主按钮 | 高 36，水平 padding 12，圆角 1 | 同桌面 | 收入菜单 |
...
导航链接 14 / 21.7、400；菜单链接 18 / 24、400；主按钮 Geist Mono、13 / 20、500。操作区 gap 10。语言菜单宽 148、padding 4、圆角 8，选项最小高 44、文字 14 / 20，选中项字重 500。
...
| Hero 入场 | 标题、说明和按钮整组随 Canvas 扫描线从上到下显露；遮罩共用扫描进度，不叠加淡入、位移或独立计时的入场动画。下方 Product 标题区在扫描结束后显示；直接进入下方内容时不等待扫描。减少动态效果时直接显示 |
