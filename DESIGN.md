...
Excel 工作表居中显示 A–D 列及第 1–11 行，内容不可编辑或选中；亮色模式下保留左侧行号，表头、行列标题和 Sheet 栏底色使用 `#F7F7F7`。暗色模式隐藏左侧行号及其占位列，工作表外框内、单元格、表头和 Sheet 栏均无填充色，保留网格线及文字。Sheet 切换不显示绿色下划线，不提供 Open workbook 链接。桌面和平板上，从工作表底部中央引出一条连线，再分成三条连接下方 Table。
...
Product 文档演示沿用页面宽度断点，不使用视口高度判断档位：宽度 768–1199px 时采用静态布局，宽度 ≥1200px 时使用桌面滚动演示，手机保持单列。桌面滚动演示仅固定格式 Tab 与文档画布，Product 标题、说明和格式链接随页面正常滚走；固定后格式 Tab 与导航下沿保持 40px 间距。宽度 768–1439px 时，PDF、Word、Excel 共用居中的 1280px 文档画布，按可用宽度等比缩小、不放大，避免切换设备档位或格式 Tab 时文档卡片突然变大；三个来源区域保持三列，两个来源区域保持两列。
